  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (url.pathname === '/api/submissions' && request.method === 'POST') {
        ...
        if (!body?.playerId || !body?.playerName || !body?.day || !/^\d{6}$/.test(body?.playerPin || '')) return json({ error: 'Missing required submission fields or invalid 6-digit code.' }, { status: 400 });
        const player = await env.DB.prepare(`SELECT id, pin_code FROM players WHERE id = ?`).bind(body.playerId).first<any>();
