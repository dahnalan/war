...
type SubmissionBody = { id: string; playerName: string; guildRole?: string; day: string; spendProfile?: string; confidence?: string; guaranteed?: number; maximum?: number; notes?: string; slots?: string[]; inputs?: Record<string, number>; createdAt?: string };
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === '/api/submissions' && request.method === 'POST') {
      ...
      if (!body?.id || !body?.playerName || !body?.day) return json({ error: 'Missing required submission fields.' }, { status: 400 });
      ...
      await env.DB.prepare(`INSERT INTO submissions (id, player_name, guild_role, vs_day, spend_profile, confidence, guaranteed_points, maximum_points, notes, slots_json, inputs_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(body.id, body.playerName, body.guildRole || '', body.day, body.spendProfile || 'free-only', body.confidence || 'medium', guaranteed, maximum, body.notes || '', JSON.stringify(body.slots || []), JSON.stringify(body.inputs || {}), body.createdAt || new Date().toISOString()).run();
      return json({ ok: true });
    }
